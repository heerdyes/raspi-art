...
class ParticleEmitter {
  ...
  void updatep(Vector2 x) {
    p=x;
  }

  void updatep(int mx, int my) {
    updatep(new Vector2((float)mx, (float)my));
  }
}
...
class ParticleSystem {
  ...
  void render(float t, PGraphics g) {
    for (int i=0; i<swarm.size(); i++) {
      ...
      p.blip(t);
      for (Field fx : fields) {
        p.applyforce(fx.evaluate(p.p, t));
      }
    }
  }

  void updatefieldorigins(Vector2 xorig) {
    for (Field f : fields) {
      f.updateorigin(xorig);
    }
  }

  void updatefieldorigins(int mx, int my) {
    updatefieldorigins(new Vector2((float)mx, (float)my));
  }
}
