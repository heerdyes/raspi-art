...
class Root{
  ...
  void grow(int c){
    ...
    this.ltroot=new Root(x1,y1,this.br*this.r,pow(-1,c+1)*this.br,this.a+this.ba,pow(-1,c)*this.ba);
    this.rtroot=new Root(x1,y1,this.br*this.r,pow(-1,c)*this.br,this.a-this.ba,pow(-1,c+1)*this.ba);
...
class Whorl{
  ...
  Whorl(float x,float y,float n,float di,float fr,float fa){
    this.roots=new ArrayList<Root>();
    float angle=2*PI/n;
    for(int i=0;i<n;i++){
      //this.roots.add(new Root(x,y,di,fr,i*angle,fa));
      this.roots.add(new Root(x,y,di,fr,i*angle+PI/2,fa));
    }
  }
...
void setup(){
  ...
  noFill();
  smooth();
  mainwhorl=new Whorl(width/2,height/2,4,130,0.64,PI/4); // v0.8
